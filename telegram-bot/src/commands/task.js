...
export function getEntityCache() {
    ...
    globalEntityCache = {};
    try {
        console.log('[DEBUG] getEntityCache invoked.');
        ...
        const potentialPaths = [
            ...
            '/app/src/entities', // Docker absolute
            ...
            '/app/st4cker/entities' // Docker absolute old
        ];
        ...
        for (const p of potentialPaths) {
            try {
                if (fs.existsSync(p)) {
                    console.log(`[DEBUG] Found entities at: ${p}`);
                    entitiesDir = p;
                    break;
                } else {
        ...
        if (entitiesDir) {
            ...
            console.log(`[DEBUG] Files found: ${files.join(', ')}`);
            ...
            files.forEach(file => {
                try {
                    ...
                    console.error(`[DEBUG] Failed to parse ${file}:`, err.message);
                }
            });
        } else {
            console.error('[Entity Loader] NO ENTITY DIRECTORY FOUND!');
        }
    } catch (e) {
        console.error('[Entity Loader] Critical Failure:', e.message);
    }
