...
app.on('ready', async () => {
    ...
    async function syncToVPS(method: string, apiPath: string, body?: any): Promise<boolean> {
        ...
        try {
            ...
            console.log(`[VPS Sync] Offline or error: ${(e as Error).message}`);
            return false;
        }
    }

    function queueSync(entityType: string, action: string, entityId: string, payload: any) {
        try {
            const db = getDB();
            db.prepare(`INSERT OR REPLACE INTO sync_queue (id, entity_type, action, entity_id, payload, created_at, synced) VALUES (?, ?, ?, ?, ?, ?, 0)`)
                .run(randomUUID(), entityType, action, entityId, JSON.stringify(payload), new Date().toISOString());
    ...
    async function processQueue() {
        const db = getDB();
        const pending = db.prepare('SELECT * FROM sync_queue WHERE synced = 0 ORDER BY created_at').all() as any[];
    ...
    async function pullFromVPS() {
        try {
            ...
            if (projectsRes.ok) {
                const { data: vpsProjects } = await projectsRes.json() as any;
            ...
            if (txRes.ok) {
                const { data: vpsTx } = await txRes.json() as any;
                ...
                telegramSocket.on('telegram-event', async (event: any) => {
                    ...
                    try {
                        ...
                        if (event.eventType === 'task.created') {
                            ...
                            if (rest.course) updates.course = rest.course;
                            ...
                            if (Object.keys(updates).length > 0) {
                                try {
                                    const result = assignments.update(id, updates);
                            ...
                            if (transactions.update(id, dbUpdates)) {
                                new (require('electron').Notification)({
                            ...
                            const dbUpdates: any = {};
                            if (updates.name) dbUpdates.title = updates.name; // Map name -> title
                            ...
                            if (updates.status) {
                                let dbStatus = 'active';
                                if (updates.status === 'completed') dbStatus = 'completed';
                                if (updates.status === 'on_hold') dbStatus = 'on_hold';
                                if (updates.status === 'in_progress') dbStatus = 'active'; // Map specifically
                                dbUpdates.status = dbStatus;
                            }

                            if (Object.keys(dbUpdates).length > 0) {
                                try {
                                    if (projects.update(id, dbUpdates)) {
                                        new (require('electron').Notification)({
                            ...
                            const getValue = (field: any) => (field && typeof field === 'object' && field.value !== undefined) ? field.value : field;
                            ...
                            if (id && updates) {
                                ...
                                transactions.update(id, dbUpdates);
                                success = true;
                            }
                        }
            ...
            if (data.success) {
                ...
                setTimeout(() => {
                    if (telegramStore && telegramSocket) {
                        ...
                        syncUserDataToBackend(telegramStore, telegramSocket).catch(console.error);
                    }
                }, 2000);
